...
namespace Elffy.Threading
{
    public static class SingleScreenTaskExtension
    {
        ...
        /// <summary>指定の <see cref="Task{TResult}"/> の後に Dispatcher スレッドで処理を行います。</summary>
        ...
        public static void ContinueWithDispatch<T>(this Task<T> task, Action<T> action)
        {
            ...
            task.ContinueWith((t, state) =>
            {
                ...
                if(t.Exception is null) {
                    Dispatcher.Invoke(() => action(t.Result));  // captuer t, action
                }
                else {
                    throw t.Exception;
                }
        ...
        /// <param name="task">ソースになるタスク</param>
        /// <param name="action">Dispatcher スレッドで行う処理</param>
        public static void ContinueWithDispatch<T>(this Task<T> task, Action<Task, T> action)
        {
            if(task is null) { throw new ArgumentNullException(nameof(task)); }
            if(action is null) { throw new ArgumentNullException(nameof(action)); }
            ...
            task.ContinueWith((t, state) =>
            {
                Debug.Assert(state is Action<Task, T>);
                var action = Unsafe.As<Action<Task, T>>(state);

                if(t.Exception is null) {
                    Dispatcher.Invoke(() => action(t, t.Result));   // capture t, action
                }
                else {
                    throw t.Exception;
                }
