...
namespace UnitTest
{
    public class ResourceTest
    {
        ...
        public void ResourceLoadTest()
        {
            ...
            var package = new LocalResourcePackage("Test", "Resources.dat");
            var pair = package.GetResourceNames().Select(x => (Resource: x, Original: sourceNames.Find(y => x == y)))
                                .ToArray();
            foreach(var (name, original) in pair) {
                byte[] hash1;
                byte[] hash2;
                if(package.TryGetStream(name, out var stream1) == false) {
                    throw new Exception();
                }
