...
namespace UnitTest
{
    public class ValueTypeRentMemoryTest
    {
        [Theory]
        [InlineData(1)]
        [InlineData(2)]
        ...
        [Theory]
        [InlineData(1024 * 1024 * 1024 / sizeof(int))]
        [InlineData(int.MaxValue / sizeof(int))]
        ...
        public void Invalid()
        {
            ...
            {
                new ValueTypeRentMemory<int>(-1, true);
            });
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new ValueTypeRentMemory<int>(-1, false);
            });

            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new ValueTypeRentMemory<int>(-10, true);
            });
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new ValueTypeRentMemory<int>(-10, false);
            });

            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new ValueTypeRentMemory<int>(int.MaxValue, true);
            });
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new ValueTypeRentMemory<int>(int.MaxValue, false);
            });
        }
